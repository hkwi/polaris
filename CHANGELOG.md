...
- Iceberg tables now pin `encryption.key-id` in catalog state and reject adding, changing, or
  removing it after table creation, preserving the table's encryption key identity. Updates to
  tables created before this pin was recorded are rejected; supporting them requires a separate
  trusted migration mechanism.
